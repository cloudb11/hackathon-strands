...
A conversational AI assistant with **RAG (Retrieval-Augmented Generation)**, **multi-turn conversation memory**, and **persistent session storage** — built with the [Strands Agents SDK](https://github.com/strands-agents/sdk-python).
...
1. User sends a question (with optional `session_id`)
2. Backend auto-retrieves relevant KB chunks via `retrieve_context()`
3. KB context is injected into the prompt as a `[KNOWLEDGE BASE CONTEXT]` block
4. The per-session Strands `Agent` processes the augmented question with full conversation history
5. Agent can also call `search_knowledge_base` tool for additional targeted searches
6. Response is returned with the `session_id` for follow-up messages
